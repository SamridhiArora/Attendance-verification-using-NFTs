...
// Initialize devnet account address
const DEVNET_ACCOUNT = '0x060605f4a3ff7c6cc6563f1d4b864a19a259aa3f36aef78494681f578e7cc38e';
...
// Get wallet balance from devnet
async function getWalletBalance() {
    try {
        const account = await window.petra.account();
        const balance = await window.petra.getBalance();
        const devnetBalance = await window.petra.getBalance(DEVNET_ACCOUNT);
        
        // Update devnet balance
        document.getElementById('devnetBalance').textContent = (devnetBalance / 100000000).toFixed(2);
        document.getElementById('devnetBalance').style.color = devnetBalance > 0 ? 'var(--secondary-color)' : '#ff6b6b';
        
        // Update user balance
        document.getElementById('walletBalance').textContent = (balance / 100000000).toFixed(2);
        document.getElementById('walletBalance').style.color = balance > 0 ? 'var(--secondary-color)' : '#ff6b6b';
    } catch (error) {
        console.error('Error getting wallet balance:', error);
        document.getElementById('devnetBalance').textContent = 'Error';
...
// Handle certificate issuance with token transfer
const handleCertificateSubmit = async(e) => {
    ...
    if (isNaN(tokenAmount) || tokenAmount < 1) {
        showToast('Please enter a valid token amount (minimum 1 APT)', 'error');
        return;
    }
    ...
    try {
        ...
        const devnetBalance = await window.petra.getBalance(DEVNET_ACCOUNT);
        const devnetBalanceInAPT = devnetBalance / 100000000;
        
        if (devnetBalanceInAPT < tokenAmount) {
            showToast(`Insufficient devnet balance. You need ${tokenAmount} APT but only have ${devnetBalanceInAPT.toFixed(2)} APT`, 'error');
            return;
        }
        
        // Prepare token transfer transaction from devnet
        const transaction = {
            type: "entry_function_payload",
            function: "0x1::coin::transfer",
            type_arguments: ["0x1::aptos_coin::AptosCoin"],
            arguments: [await window.petra.account(), tokenAmount * 100000000] // Transfer from devnet to user
        };

        // Execute token transfer
        const response = await window.petra.signAndSubmitTransaction(transaction);
        console.log('Token transfer transaction:', response);
        ...
        const certificateData = {
            ...
            devnet_balance_used: devnetBalanceInAPT
        };
        ...
        showToast(`Certificate issued successfully! ${tokenAmount} APT transferred from devnet.`, 'success');
        e.target.reset();
...
// Update recent certificates table with token amount
        ...
        <tr class="fade-in" style="animation-delay: ${index * 0.1}s">
            ...
            <td>
                ...
                <span class="badge bg-info ms-2">
                    <i class="fas fa-coins"></i> ${cert.token_amount || 0}
                </span>
                <span class="badge bg-info ms-2">
